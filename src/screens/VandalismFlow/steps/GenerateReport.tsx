...
import type { VandalismStackParamList, RootStackParamList } from '../../../navigation/types';
import type { NativeStackNavigationProp } from '@react-navigation/native-stack';
import StepHeader from '../../../components/StepHeader';
import {
  markIncidentComplete, getIncidentPhotos, getIncidentSteps,
...
type Props = NativeStackScreenProps<VandalismStackParamList, 'GenerateReport'>;
...
export default function GenerateReport({ navigation }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setStepCount(steps.filter(s => s.completed).length);
    })();
  }, [activeIncident]);
  ...
  const handleGeneratePDF = async () => {
    ...
    try { await generateIncidentPDF(activeIncident.id); }
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <StepHeader title="Incident Report" step={8} total={8} onBack={() => navigation.goBack()} />
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.successBadge}>
          <Text style={styles.successIcon}>✅</Text>
          <Text style={styles.successTitle}>Documentation Complete</Text>
          <Text style={styles.successSub}>{formatDateTime(Date.now())}</Text>
        </View>
        ...
        <View style={styles.pdfSection}>
          ...
          <Text style={styles.pdfText}>
            A full insurance-ready PDF with all your photos, police report number, location, and notes —
            generated entirely on this device, no internet required.
          </Text>
          <TouchableOpacity style={[styles.pdfButton, generatingPDF && styles.disabled]} onPress={handleGeneratePDF} disabled={generatingPDF}>
            {generatingPDF ? <ActivityIndicator color="#FFF" /> : <Text style={styles.pdfButtonText}>Generate PDF Report</Text>}
          </TouchableOpacity>
        </View>

        <View style={styles.infoBox}>
          <Text style={styles.infoText}>
            Your documentation is saved securely on this device. View it anytime in Incident History.
          </Text>
        </View>
      </ScrollView>

      <View style={styles.footer}>
        <TouchableOpacity style={[styles.doneButton, finishing && styles.disabled]} onPress={handleDone} disabled={finishing}>
          {finishing ? <ActivityIndicator color="#FFF" /> : <Text style={styles.doneText}>Done — Back to Home</Text>}
        </TouchableOpacity>
      </View>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  successTitle: { fontSize: FONT.heading, fontFamily: 'Poppins_700Bold', fontWeight: '700', color: COLORS.success },
  ...
  statNumber: { fontSize: 36, fontFamily: 'Poppins_800ExtraBold', fontWeight: '800', color: COLORS.primary },
  ...
  infoBox: { backgroundColor: COLORS.primaryLight, borderRadius: RADIUS.md, padding: SPACING.md },
  infoText: { fontSize: FONT.body, color: COLORS.primary, lineHeight: 22 },
  footer: {
    padding: SPACING.lg, paddingBottom: SPACING.xl,
    backgroundColor: COLORS.surface, borderTopWidth: 1, borderTopColor: COLORS.border,
  },
  doneButton: { backgroundColor: COLORS.success, borderRadius: RADIUS.lg, paddingVertical: SPACING.md, alignItems: 'center' },
