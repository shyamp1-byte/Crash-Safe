...
type Props = NativeStackScreenProps<VandalismStackParamList, 'DamagePhotos'>;
...
export default function DamagePhotos({ navigation }: Props) {
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <StepHeader title="Damage Photos" step={3} total={8} onBack={() => navigation.goBack()} />
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <Text style={styles.heading}>Photograph all damage</Text>
        <PhotoCapture
          photos={photos}
          onPhotosChange={setPhotos}
          hint="Capture every damaged area — keying, broken glass, graffiti, slashed tires. Include close-ups and wide shots."
        />
      </ScrollView>
