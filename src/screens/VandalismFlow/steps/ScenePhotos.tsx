...
type Props = NativeStackScreenProps<VandalismStackParamList, 'ScenePhotos'>;
...
export default function ScenePhotos({ navigation }: Props) {
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <StepHeader title="Scene Photos" step={4} total={8} onBack={() => navigation.goBack()} />
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <Text style={styles.heading}>Photograph the scene</Text>
        <PhotoCapture
          photos={photos}
          onPhotosChange={setPhotos}
          hint="Capture the surrounding area, street signs, any tools or objects left behind, nearby security cameras or businesses."
        />
      </ScrollView>
      <View style={styles.footer}>
        <TouchableOpacity style={styles.skipButton} onPress={() => navigation.navigate('PoliceReport')}>
