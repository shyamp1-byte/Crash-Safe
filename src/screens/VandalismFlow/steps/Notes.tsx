...
type Props = NativeStackScreenProps<VandalismStackParamList, 'Notes'>;

const PROMPTS = [
  'Approximate time you discovered the vandalism?',
  'Anyone in the area who may have witnessed it?',
  'Any security cameras nearby (businesses, parking lots)?',
  'Description of any suspects or vehicles seen?',
...
export default function Notes({ navigation }: Props) {
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <StepHeader title="Notes" step={7} total={8} onBack={() => navigation.goBack()} />
      <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <View style={styles.content}>
          ...
          <View style={styles.prompts}>
            {PROMPTS.map((p, i) => <Text key={i} style={styles.prompt}>• {p}</Text>)}
          </View>
        ...
        <View style={styles.footer}>
          <TouchableOpacity style={styles.skipButton} onPress={() => navigation.navigate('GenerateReport')}>
          ...
          >
            {saving ? <ActivityIndicator color="#FFF" /> : <Text style={styles.nextText}>Save & Continue</Text>}
          </TouchableOpacity>
        </View>
...
const styles = StyleSheet.create({
  ...
  heading: { fontSize: FONT.heading, fontFamily: 'Poppins_700Bold', fontWeight: '700', color: COLORS.text, marginBottom: SPACING.xs },
  subheading: { fontSize: FONT.body, color: COLORS.textMuted, marginBottom: SPACING.md, lineHeight: 22 },
  prompts: {
  ...
  textarea: {
    ...
    borderRadius: RADIUS.md, padding: SPACING.md, fontSize: FONT.body, color: COLORS.text,
