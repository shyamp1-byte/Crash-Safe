...
type Props = NativeStackScreenProps<HitRunStackParamList, 'ScenePhotos'>;
...
export default function ScenePhotos({ navigation }: Props) {
  ...
  const handleNext = async () => {
    ...
    try {
      await Promise.all(
        photos.map(p =>
          addPhoto(activeIncident.id, p.uri, 'scene', null, null),
        ),
      );
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      >
        ...
        <PhotoCapture
          ...
          hint="Capture the surrounding area, street signs, parking lot, any paint transfer or debris left by the other vehicle."
        />
      </ScrollView>
