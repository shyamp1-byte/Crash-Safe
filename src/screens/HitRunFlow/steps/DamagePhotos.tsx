...
type Props = NativeStackScreenProps<HitRunStackParamList, 'DamagePhotos'>;
...
export default function DamagePhotos({ navigation }: Props) {
  ...
  const handleNext = async () => {
    ...
    try {
      await Promise.all(
        photos.map(p =>
          addPhoto(activeIncident.id, p.uri, 'damage', null, null),
        ),
      );
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      >
        <Text style={styles.heading}>Photograph the damage</Text>
        <PhotoCapture
          photos={photos}
          onPhotosChange={setPhotos}
          hint="Capture all damage areas, license plate, and every side of your vehicle."
        />
      </ScrollView>
      <View style={styles.footer}>
        <TouchableOpacity
          style={styles.skipButton}
          onPress={() => navigation.navigate('ScenePhotos')}
        >
