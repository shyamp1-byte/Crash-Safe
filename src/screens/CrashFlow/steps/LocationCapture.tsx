...
type Props = NativeStackScreenProps<CrashStackParamList, 'LocationCapture'>;
...
export default function LocationCapture({ navigation }: Props) {
  ...
  const confirm = async (label: string, finalLat: number, finalLng: number) => {
    ...
    try {
      ...
      navigation.navigate('YourVehiclePhotos');
    } finally {
      setSaving(false);
    }
  };
      ...
      >
        ...
        <View style={styles.fieldGroup}>
          ...
          <AddressAutocomplete
            ...
            onChangeText={text => {
              setAddress(text);
              // Clear GPS coords only if user diverges from auto-filled label (saves wrong coords otherwise)
