...
type Props = NativeStackScreenProps<CrashStackParamList, 'YourVehiclePhotos'>;

export default function YourVehiclePhotos({ navigation }: Props) {
  ...
  const handleNext = async () => {
    ...
    try {
      await Promise.all(
        photos.map(p =>
          addPhoto(activeIncident.id, p.uri, 'your_vehicle', null, null),
        ),
      );
      completeStep('your_vehicle_photos');
    } finally {
      setSaving(false);
    }
    navigation.navigate('OtherDriverInfo');
  };
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      >
        <Text style={styles.heading}>Photograph your vehicle</Text>
        <PhotoCapture
          photos={photos}
          onPhotosChange={setPhotos}
          hint="Capture all sides, the license plate, and close-ups of any damage."
        />
      </ScrollView>
      <View style={styles.footer}>
        {photos.length === 0 && (
          <TouchableOpacity
            ...
            onPress={() => navigation.navigate('OtherDriverInfo')}
          >
        ...
        >
          {saving ? (
            ...
            <Text style={styles.nextText}>
              Next {photos.length > 0 ? `(${photos.length} photo${photos.length > 1 ? 's' : ''})` : ''}
            </Text>
          )}
        </TouchableOpacity>
      </View>
