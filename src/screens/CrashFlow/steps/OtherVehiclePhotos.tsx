...
type Props = NativeStackScreenProps<CrashStackParamList, 'OtherVehiclePhotos'>;

export default function OtherVehiclePhotos({ navigation }: Props) {
  ...
  const handleNext = async () => {
    ...
    try {
      await Promise.all(
        photos.map(p =>
          addPhoto(activeIncident.id, p.uri, 'other_vehicle', null, null),
        ),
      );
      completeStep('other_vehicle_photos');
    } finally {
      setSaving(false);
    }
    navigation.navigate('WitnessInfo');
  };
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      >
        <Text style={styles.heading}>Photograph the other vehicle</Text>
        <PhotoCapture
          photos={photos}
          onPhotosChange={setPhotos}
          hint="Capture license plate, all sides, VIN if visible, and all damage."
        />
      </ScrollView>
      <View style={styles.footer}>
        <TouchableOpacity
          style={styles.skipButton}
          onPress={() => navigation.navigate('WitnessInfo')}
        >
        ...
        >
          {saving ? (
            ...
            <Text style={styles.nextText}>
              Next {photos.length > 0 ? `(${photos.length})` : ''}
            </Text>
          )}
        </TouchableOpacity>
      </View>
