...
import {
  requestLocationPermission, getCurrentLocation, type LocationResult,
...
type Props = NativeStackScreenProps<WeatherStackParamList, 'LocationCapture'>;
...
export default function LocationCapture({ navigation }: Props) {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!granted) { setPermissionDenied(true); setLoading(false); return; }
      try {
        const loc = await getCurrentLocation();
        setLat(loc.lat); setLng(loc.lng); setGpsObtained(true);
        if (loc.label) { setAddress(loc.label); setGpsLabel(loc.label); }
  ...
  const confirm = async (label: string, finalLat: number, finalLng: number) => {
    ...
    setSaving(true);
    try {
      await updateIncidentLocation(activeIncident.id, { lat: finalLat, lng: finalLng, label: label.trim() } as LocationResult);
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <StepHeader title="Incident Location" step={2} total={7} onBack={() => navigation.goBack()} />
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        <Text style={styles.heading}>Where did the weather damage occur?</Text>

        {loading ? (
          <View style={styles.gpsCard}><ActivityIndicator color={COLORS.primary} /><Text style={styles.gpsLoading}>Getting your GPS location…</Text></View>
        ) : gpsObtained && !permissionDenied ? (
          <View style={styles.gpsCard}>
            <Text style={styles.gpsIcon}>📍</Text>
            <View style={styles.gpsBody}>
              <Text style={styles.gpsTitle}>GPS Captured</Text>
              {(lat !== 0 || lng !== 0) && <Text style={styles.gpsCords}>{lat.toFixed(6)}, {lng.toFixed(6)}</Text>}
              <Text style={styles.gpsNote}>Exact coordinates saved. Verify or correct the address below.</Text>
            </View>
          </View>
        ) : permissionDenied ? (
          <View style={styles.warningCard}><Text style={styles.warningText}>GPS unavailable — confirm the full address below.</Text></View>
        ) : null}
        ...
        <View style={styles.fieldGroup}>
          ...
          <AddressAutocomplete
            value={address}
            onChangeText={text => { setAddress(text); if (gpsLabel && text !== gpsLabel) { setLat(0); setLng(0); } }}
            onSelect={(label, selLat, selLng) => { setAddress(label); setLat(selLat); setLng(selLng); }}
        ...
        >
          {saving ? <ActivityIndicator color="#FFF" /> : <Text style={styles.confirmText}>Confirm Location</Text>}
        </TouchableOpacity>
      </ScrollView>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  gpsCard: {
    ...
    borderWidth: 1, borderColor: '#BFDBFE', flexDirection: 'row', gap: SPACING.md, alignItems: 'flex-start',
  },
  ...
  confirmButton: {
    backgroundColor: COLORS.primary, borderRadius: RADIUS.lg, paddingVertical: SPACING.md, alignItems: 'center',
  },
