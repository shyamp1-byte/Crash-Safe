...
type Props = NativeStackScreenProps<WeatherStackParamList, 'InsuranceGuidance'>;

const TIPS = [
  { icon: '📞', tip: 'Call your insurance company as soon as possible — storm claims surge fast' },
  { icon: '📋', tip: 'Have your policy number, photos, and location details ready when you call' },
  { icon: '🔧', tip: 'Temporary repairs to prevent further damage are OK — keep all receipts' },
  { icon: '📸', tip: 'Do NOT make permanent repairs until the adjuster completes their inspection' },
  { icon: '📄', tip: 'Request a weather report or NOAA storm record to support your claim' },
  { icon: '⏰', tip: 'File promptly — some policies have strict deadlines after weather events' },
];
...
export default function InsuranceGuidance({ navigation }: Props) {
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <StepHeader title="Contact Insurance" step={5} total={7} onBack={() => navigation.goBack()} />
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <Text style={styles.heading}>Notify your insurance company</Text>
        <Text style={styles.subheading}>
          Weather damage is covered under comprehensive coverage. This is not at-fault — file with confidence.
        </Text>
        ...
        <View style={styles.coverageBox}>
          ...
          <Text style={styles.coverageItem}>
            • <Text style={styles.bold}>Comprehensive coverage</Text> — covers all weather damage; deductible applies
          </Text>
          <Text style={styles.coverageItem}>
            • <Text style={styles.bold}>Glass coverage (if separate)</Text> — may cover cracked windshield with no deductible
          </Text>
          <Text style={styles.coverageItem}>
            • <Text style={styles.bold}>Gap insurance (if financed)</Text> — covers difference if car is totaled and worth less than owed
          </Text>
        </View>
      </ScrollView>
